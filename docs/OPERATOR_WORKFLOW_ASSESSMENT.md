...
| Hunt | Correlate newest evidence across network scopes, hosts, devices, datasets, and SearchSploit candidates; current SearchSploit results restore automatically until evidence or the active database changes | Network-wide view by default or a run-specific link | Host, service, and SearchSploit rows can open Reach with destination/protocol/port loaded while leaving Source unchanged |
