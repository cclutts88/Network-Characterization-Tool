...
IMAGE="network-characterization-tool:0.15.8-range-20260924"
CONTAINER="nct"
CONTAINER_PORT=8445
...
[ -n "$RANGE_IP" ] || die "Usage: sh $WORKDIR/scripts/nct-start-legacy.sh RANGE_IP [ADMIN_USERNAME]"
...
printf '%s\n' "Starting NCT with the compatibility settings for the older Range Docker engine..."
if [ "$account_count" -eq 0 ]; then
    umask 077
